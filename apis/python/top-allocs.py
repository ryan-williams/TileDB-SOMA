...
@command
@option("-m", "--markdown", is_flag=True, help="Output a Markdown list wrapped in a `<div>` tag (suitable for embedding directly in a Markdown file)")
@option("-t", "--tiledbsoma-ref", help="Source links will use this TileDB-SOMA ref (default: current `HEAD` SHA)")
@argument("stats_path")
def main(
    markdown: bool,
    tiledbsoma_ref: str | None,
    stats_path: str,
):
    ...
    top_allocs = stats.top_allocs if 'top_allocs' in stats else stats.top_allocations_by_size
