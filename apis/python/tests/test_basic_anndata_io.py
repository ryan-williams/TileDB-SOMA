...
from ._util import TESTDATA, assert_adata_equal, assert_array_equal, make_pd_df
    ...
    [
        *[
            ...
            for obs_query, var_query, shape, nnz in [
                ...
                (
                    AxisQuery(),
                    filter('attr("vst.variance.standardized") > 2.0'),
                    (80, 5),
                    400,
                ),
                (
...
def test_dask_query(
    ...
    assert len(obs_chunks) == (nobs + dask_chunk_size - 1) // dask_chunk_size
    assert var_chunks == (nvar,)
    X2 = ad2.X.compute()
